...
# Solo modificar si hay un reflejo de color muy fuerte que interfiere.

WHITE_S_MIN = 0
WHITE_S_MAX = 30
# Saturación del blanco es MUY baja (color casi sin tono).
# (+) Subir S_max → detecta grises claros y superficies levemente coloreadas.
#     Riesgo: el piso gris o tiles claros pueden aparecer como "blanco".
# (-) Bajar S_max → solo detecta blanco puro. Si la línea parece gris
#     en cámara, puede dejar de detectarse (x_white = None → error NaN).

WHITE_V_MIN = 180
WHITE_V_MAX = 255
# Brillo del blanco es MUY alto.
# (+) Subir V_min → solo detecta superficies muy brillantes. Más selectivo.
# (-) Bajar V_min → detecta blancos más oscuros/sombreados. Útil con poca luz,
#     pero puede confundir el piso gris claro con la línea blanca.

# Amarillo — eje central discontinuo
YELLOW_H_MIN = 20
YELLOW_H_MAX = 35
# Tono del amarillo en OpenCV: aprox 20-35 (escala 0-180).
# (+) Ampliar rango (bajar min o subir max) → más tolerante a variaciones de luz.
#     Riesgo: captura naranja (H<20) o verde-amarillento (H>40).
# (-) Estrechar rango → más selectivo, menos falsos positivos.
#     Riesgo: con iluminación cálida el amarillo puede correrse y no detectarse.

YELLOW_S_MIN = 100
YELLOW_S_MAX = 255
# Saturación del amarillo es alta (color vivo).
# (+) Subir S_min → exige amarillo más puro/saturado. Si la luz "lava" el color,
#     la línea puede desaparecer (x_yellow = None).
# (-) Bajar S_min → detecta amarillos más pálidos. Puede capturar el piso
#     si tiene un tinte amarillento.

YELLOW_V_MIN = 100
YELLOW_V_MAX = 255
# Brillo del amarillo.
# (+) Subir V_min → exige más brillo. Puede perder la línea en zonas de sombra.
# (-) Bajar V_min → detecta amarillos más oscuros. Riesgo de confundir con piso.
...
# (+) Subir → el robot asume un carril más ancho → estima el centro más alejado.
# (-) Bajar → estimaciones más conservadoras del centro del carril.
# NO cambiar salvo que el carril físico sea diferente al estándar.

PX_PER_METER = 600.0
# Escala de la vista IPM: píxeles por metro en la imagen de pájaro.
# Determina cómo se convierte el desplazamiento en píxeles a metros.
# (+) Subir → errores laterales más pequeños → reacciona menos → puede no
#     corregir suficiente en curvas.
# (-) Bajar → errores laterales más grandes → reacciona más fuerte.
# Calibrar midiendo un objeto de tamaño conocido en la vista de pájaro.

LOOK_AHEAD_ROW = 0.6
# Fila de la imagen IPM donde se mide el error: 0.0=arriba (lejos), 1.0=abajo (cerca).
# Define cuánto "hacia adelante" mira el robot para medir el error lateral.
# (+) Subir (más abajo = más cerca del robot) → más reactivo pero puede oscilar.
# (-) Bajar (más arriba = más lejos) → anticipa más las curvas → trayectoria
#     más suave, pero reacciona más tarde a cambios bruscos.

# ── CÁMARA / IPM — TRAPECIO DE PERSPECTIVA ────────────────────────────────────
# Estos puntos definen el trapecio del piso que se convierte a vista de pájaro.
# Son fracciones del ancho (w) y alto (h) de la imagen.
# Ajustar si las líneas aparecen curvas en /lane/debug_image.

IPM_TOP_Y = 0.50
# Altura del borde SUPERIOR del trapecio (fracción de h, 0=tope, 1=fondo).
# (+) Subir → captura menos piso, como si la cámara fuera más horizontal.
# (-) Bajar → captura más piso. Ideal para cámara inclinada más hacia abajo.
...
# (+) Acercar los puntos → trapecio más estrecho arriba → franja más angosta.
...
def build_ipm(w, h):
    ...
    Esto permite convertir desplazamiento en píxeles a metros reales.
    """
    src = np.float32([
        [IPM_TOP_LEFT_X  * w, IPM_TOP_Y    * h],  # esquina superior izquierda
        [IPM_TOP_RIGHT_X * w, IPM_TOP_Y    * h],  # esquina superior derecha
        [1.00            * w, IPM_BOTTOM_Y * h],  # esquina inferior derecha
        [0.00            * w, IPM_BOTTOM_Y * h],  # esquina inferior izquierda
    ])
    dst = np.float32([
        [0.30 * w, 0.0],  # superior izquierda (vista de pájaro)
        [0.70 * w, 0.0],  # superior derecha
        [0.70 * w, h  ],  # inferior derecha
        [0.30 * w, h  ],  # inferior izquierda
    ])
...
def centroid_x(mask, min_area):
    ...
    Si el área de píxeles encendidos es menor que min_area, retorna None
    (se descarta como ruido — no hay línea real en esa zona).
    """
...
def apply_hsv_masks(frame, white_lo, white_hi, yellow_lo, yellow_hi, M):
    ...
    Pasos:
      ...
      4. Limpia el ruido con morfología MORPH_OPEN (erosión + dilatación).
    """
    h, w = frame.shape[:2]
...
def compute_lane_error(mask_white, mask_yellow, w, h):
    ...
    Casos manejados:
      Ambas líneas visibles → centro = promedio entre blanca y amarilla.
    ...
    Convención de signo:
      error > 0 → centro del carril a la DERECHA → girar derecha (omega < 0).
      error < 0 → centro del carril a la IZQUIERDA → girar izquierda (omega > 0).
    """
...
def _nothing(_):
    """Callback vacío requerido por cv2.createTrackbar."""
    pass


def _make_trackbars(window, h_min, h_max, s_min, s_max, v_min, v_max):
    """Crea una ventana con 6 sliders HSV (H min/max, S min/max, V min/max)."""
...
def _read_trackbars(window):
    """Lee los 6 valores HSV actuales de los sliders de una ventana."""
...
def run_calibrator(source):
    ...
    Controles:
      ...
      s       : guardar hsv_params_calibrado.yaml en el directorio actual.
    """
    ...
    # Crear ventanas con sliders (valores iniciales = variables del inicio)
    ...
    while True:
        ...
        # Dibujar centroides sobre la vista de pájaro
        row  = int(LOOK_AHEAD_ROW * h)
        band = slice(max(0, row - 8), min(h, row + 8))
...
class LaneDetector(Node):
    def __init__(self):
        ...
        # Rangos HSV desde las variables configurables del inicio del archivo
        ...
        self.M = None  # Matriz IPM, se calcula con el primer frame
    ...
    def on_image(self, msg):
        """
        Callback que se ejecuta por cada frame de la cámara.

        1. Convierte el mensaje ROS a imagen OpenCV (BGR).
        2. Construye la IPM en el primer frame (una sola vez).
        3. Aplica la IPM y las máscaras de color HSV.
        4. Calcula el error lateral con compute_lane_error().
        5. Publica el error en /lane_error y la imagen debug en /lane/debug_image.
        """
    ...
    def _publish_debug(self, warp, row, xw, xy, xc, header_msg):
        """
        Dibuja sobre la imagen de pájaro y publica en /lane/debug_image.

        Línea verde  = fila de muestreo (look-ahead).
        Punto blanco = centroide del borde blanco.
        Punto cian   = centroide del eje amarillo.
...
class LaneController(Node):
    def __init__(self):
        ...
        # Estado del contador de vueltas (usa odometría)
        self.laps_done       = 0
        self.total_dist      = 0.0
        ...
        self.get_logger().info(
            f'Misión: {NUM_VUELTAS} vueltas x {METROS_POR_VUELTA} m = '
            f'{NUM_VUELTAS * METROS_POR_VUELTA:.1f} m totales.')

    def on_error(self, msg):
        """
        Recibe el error lateral de /lane_error.
        Solo actualiza si el valor no es NaN (líneas detectadas correctamente).
        """
    ...
    def on_odom(self, msg):
        """
        Recibe odometría y acumula la distancia recorrida para contar vueltas.
        Cuando se completan NUM_VUELTAS, activa la señal de misión completa.
        """
    ...
    def control_loop(self):
        """
        Lazo de control PID — se ejecuta a CONTROL_RATE Hz.

        Si misión completa    → frena definitivamente.
        Si sin error reciente → frena por seguridad (timeout).
        Si error válido       → calcula omega con PID y publica /cmd_vel.

        PID:
          P = KP * error
          I = KI * integral  (con anti-windup: clamp a ±INTEGRAL_LIMIT)
        ...
        # Misión completa → frenar
        if self.mision_completa:
            self.pub.publish(Twist())
            return

        # Timeout de seguridad → frenar si no llega error
        ...
        # Término P: respuesta al error actual
        p_term = KP * error

        # Término I: acumula el error (corrige sesgo) con límite anti-windup
        self.integral += error * dt
        self.integral  = max(-INTEGRAL_LIMIT, min(INTEGRAL_LIMIT, self.integral))
        i_term = KI * self.integral

        # Término D: amortigua el cambio del error (anticipa corrección)
        d_term = KD * (error - self.last_error) / dt

        # Velocidad angular total, saturada al máximo permitido
...
def main(args=None):
    ...
    Modo calibrar (--calibrar --source 0):
      Abre el calibrador HSV interactivo sin necesitar ROS2.
      Ajusta los sliders, presiona 's' para guardar el YAML resultante.

    Modo normal (sin argumentos):
      Lanza LaneDetector + LaneController en paralelo con ROS2.
    ...
    parser.add_argument('--calibrar', action='store_true',
                        help='Modo calibración HSV interactivo (sin ROS2)')
    parser.add_argument('--source', default='0',
                        help='Índice de cámara (0,1,...) o ruta a video/imagen')
    ...
    try:
        ...
        controller.pub.publish(Twist())  # Frena el robot al salir
